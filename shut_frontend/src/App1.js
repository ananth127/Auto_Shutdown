...
function App() {
  ...
  const generateExecutable = async () => {
    try {
      await axios.post('http://localhost:5000/generate', { uniqueId });
  ...
  const toggleShutdown = async () => {
    try {
      ...
      await axios.post('http://localhost:5000/toggle-shutdown', { uniqueId, shutdown: newShutdownState });
  ...
  const fetchShutdownState = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/get-shutdown/${uniqueId}`);
